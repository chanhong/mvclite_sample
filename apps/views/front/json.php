...
use Mvclite\CUtil;
  ...
  pln('here');
  pln($qsa,'qsa');  
  $PageData["Title"] = "Json Sample";
//$dbenv = CCore::$DbEnv("dbacct");
$ajax_qs = CUtil::Tap2Qs("/odata/_json") . "&c="; // refine ajx with &c=cmd
  $msg = $PageData["Title"];
  pln($msg,'in');
  pln($ajax_qs,'aj');
      ...
      $(document).ready(function () { // load json file using jquery ajax
        $.getJSON("<?= $ajax_qs;?>", function (data) {
          ...
          $.each(data, function (key, val) {
            output += '<li><a href="' + val.age + '" style="color: rgb(51, 102, 102);" target="_blank">' + val.name + '</a></li>';
          });
          output += '</ul>';
